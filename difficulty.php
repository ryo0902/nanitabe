...
try {
  ...
  if (!isset($_SESSION['user_name']) && !isset($_SESSION['id'])) {
    ...
    if(empty($_GET['max'])&& empty($_GET['min'])) {
      ...
      throw new Exception('レベルの上限が下限を上回っています。入力しなおしてください。');
    }else{
      $max = (int) $_GET['max'];
      $min = (int) $_GET['min'];
      require("php/pdo_mysql.php");
      $sql = "SELECT * FROM recipes WHERE difficulty >= $min AND difficulty <= $max ";
 ...
 <html lang="ja">
   <head>
     <meta charset="utf-8">
     <title>LEVEL：<?php echo $min.'～'.$max ; ?>のレシピページ</title>
   ...
   <?php else: ?>
         ...
         <main>
           <div class="main_side">
         <h2>難易度のLEVEL<?php echo $min.'～'.$max ; ?>のレシピ一覧</h2>
